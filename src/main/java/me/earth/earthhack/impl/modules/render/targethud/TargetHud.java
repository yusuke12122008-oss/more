...
public class TargetHud extends Module {
    ...
    public final ColorSetting bgColor = register(new ColorSetting("BackGround", new Color(0, 39, 166, 180)));
    public final ColorSetting fColor = register(new ColorSetting("TextColor", new Color(230, 230, 230, 220)));
    protected final Setting<THud> style = register(new EnumSetting<>("Style", THud.Mode1));
    protected final Setting<TargetType> targeting = register(new EnumSetting<>("Targeting", TargetType.None));
    protected final Setting<Integer> maxSetting = register(new NumberSetting<>("Detect-Range", 10, 0, 30));
    protected final Setting<Float> posX = register(
            new NumberSetting<>("PosX", 700.0f, 0.0f, (float) Render2DUtil.CSWidth()));
    protected final Setting<Float> posY = register(
            new NumberSetting<>("PosY", 200.0f, 0.0f, (float) Render2DUtil.CSHeight()));
    /*
     * protected final Setting<Float> scale =
     * register(new NumberSetting<>("Scale", 1.0f, 0.3f, 3.0f));
     */
    protected final Setting<Boolean> pretty = register(new BooleanSetting("Pretty", true));
    protected final Setting<Boolean> ping = register(new BooleanSetting("Ping", true));
    protected final Setting<Boolean> distance = register(new BooleanSetting("Distance", true));
    protected final Setting<Boolean> phase = register(new BooleanSetting("Phase", false));
    protected final Setting<PushMode> pushMode = register(new EnumSetting<>("PhasePushDetect", PushMode.None))
            .setComplexity(Complexity.Expert);

    private static final DecimalFormat df = new DecimalFormat("0.00");

    Vec3i[] safePositions = new Vec3i[] { new Vec3i(1, 0, 0), new Vec3i(-1, 0, 0), new Vec3i(0, 0, 1),
            new Vec3i(0, 0, -1) };
    ...
    public TargetHud() {
        ...
        this.listeners.add(new LambdaListener<>(Render2DEvent.class, e -> {
            ...
            if (mc.player == null || mc.world == null || closestPlayer == null) {
                return;
            }
            ...
            if (style.getValue() == THud.Mode2) {
                ...
                if (phase.getValue() && PhaseUtil.isPhasing(closestPlayer, pushMode.getValue())) {
                    ...
                    RENDERER.drawString(name, x + ((endX - x) / 2 - nameWidth / 2.0f), y + 7,
                            fColor.getValue().getRGB());
                }
                RENDERER.drawString("HP: " + hp, x + 7, y + 25, fColor.getValue().getRGB());

                if (!Double.isNaN(protCalc)) {
                    RENDERER.drawString("Protection: " + (int) protCalc + "%", x + 7, y + 43,
                            fColor.getValue().getRGB());
                } else {
                ...
                if (distance.getValue()) {
                    RENDERER.drawString("Distance: " + df.format(closestPlayer.getDistance(mc.player)), x + 7, y + 70,
                            fColor.getValue().getRGB());
                }

                NetworkPlayerInfo playerInfo = Objects.requireNonNull(mc.getConnection())
                        .getPlayerInfo(closestPlayer.getUniqueID());
                if (playerInfo != null && ping.getValue()) {
                    RENDERER.drawString("Ping: " + playerInfo.getResponseTime() + "ms", x + 7, y + 80,
                            fColor.getValue().getRGB());
                }
                ...
                Render2DUtil.progressBar(x + (10 + (endY - y)),
                        x + (((endY - y) + hp / 36.0F * (endX - (endY - y) - x)) - 10), y + 19, 3, 0x77ff0000);
            } else if (style.getValue() == THud.Mode3) {
                if (!pretty.getValue()) {
                    Render2DUtil.drawRect(x, y, endX, endY, bgColor.getRGB());
                } else {
                ...
                Render2DUtil.progressBar(x + 7, x - 7 + (hp / 36.0F * (Math.max(nameWidth, 58))), endY - 5.0f, 5,
                        0x77ff0000);
                ...
                Render2DUtil.drawPlayerFace(closestPlayer, (int) (endX - 28),
                        (int) (y + Managers.TEXT.getStringHeight() + 2), 22, 22);
                Render2DUtil.progressBar(x + 7.5f, x - 7.5f + (hp / 36.0F * endMeasureX), endY - 2.5f, 5, 0x77ff0000);
                GL11.glPushMatrix();
                glScalef(0.5f, 0.5f, 1.0f);
                RENDERER.drawString(hp.toString(),
                        (x - 7.5f + (hp / 36.0F * endMeasureX)) / 0.5f
                                - Managers.TEXT.getStringWidth(hp.toString()) / 2.0f,
                        (endY - 4.5f) / 0.5f, fColor.getValue().getRGB());
                GL11.glPopMatrix();
            }
        }));
