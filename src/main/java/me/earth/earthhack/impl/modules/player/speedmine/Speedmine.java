...
public class Speedmine extends Module
{
    ...
    protected boolean selfSwitching;
    protected int ignoreSlotResetTicks;
    protected int lastSelectedSlot = -1;
    ...
    {
        reset();
        lastSelectedSlot = mc.player != null ? mc.player.inventory.currentItem : -1;
    }
    ...
    public void abortCurrentPos()
    {
        ...
        abortCurrentPosWithoutReset();
        reset();
    }

    public void abortCurrentPosWithoutReset()
    {
    ...
    public void tickSlotResetGuard()
    {
        if (ignoreSlotResetTicks > 0)
        {
            ignoreSlotResetTicks--;
        }

        if (mc.player != null)
        {
            lastSelectedSlot = mc.player.inventory.currentItem;
        }
    }
    ...
    public void onManualSlotChange(int newSlot)
    {
        if (mc.player == null)
        {
            return;
        ...
        if (pos == null || facing == null || PlayerUtil.isCreative(mc.player))
        {
            return;
        }
    ...
    public int beginSilentSwitch(int slot)
    {
        ...
        NetworkUtil.sendPacketNoEvent(new CPacketHeldItemChange(slot), false);
        return oldSlot;
    }

    public void endSilentSwitch(int oldSlot)
    {
        if (mc.player == null)
        {
            selfSwitching = false;
        ...
        {
            ignoreSlotResetTicks = Math.max(ignoreSlotResetTicks, 3);
            NetworkUtil.sendPacketNoEvent(new CPacketHeldItemChange(oldSlot), false);
        }
    ...
    public void tryBreak() {
        ...
        if (!pausing && ((breakSlot = findBreakSlot()) != -1 || requireBreakSlot.getValue())) {
            ...
            Locks.acquire(Locks.PLACE_SWITCH_LOCK, () ->
            {
                ...
                int oldSlot = beginSilentSwitch(breakSlot);
                ...
                endSilentSwitch(oldSlot == -1 ? lastSlot : oldSlot);
            });
    ...
    {
        int oldSlot = swap ? beginSilentSwitch(fastSlot) : -1;
