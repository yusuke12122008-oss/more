...
public class BooleanComponent extends SettingComponent<Boolean, Setting<Boolean>> {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Managers.TEXT.drawStringWithShadow(getLabel(), getFinishedX() + 5, getFinishedY() + getHeight() / 2 - (Managers.TEXT.getStringHeightI() >> 1), getBooleanSetting().getValue() ? getClickGui().get().enabledTextColor.getValue().getRGB() : getClickGui().get().disabledTextColor.getValue().getRGB());
        Render2DUtil.drawBorderedRect(getFinishedX() + getWidth() - 17,getFinishedY() + 1,getFinishedX() + getWidth() - 5,getFinishedY() + getHeight() - 1,0.5f,getBooleanSetting().getValue() ? ( hovered ? getClickGui().get().color.getValue().brighter().getRGB():getClickGui().get().color.getValue().getRGB()):(hovered ? getClickGui().get().hoverColor.getValue().getRGB():0),getClickGui().get().outlineColor.getValue().getRGB());
