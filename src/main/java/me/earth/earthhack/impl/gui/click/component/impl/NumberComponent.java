...
public class NumberComponent extends SettingComponent<Number, NumberSetting<Number>> {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Managers.TEXT.drawStringWithShadow(getLabel() + ": " + ChatFormatting.GRAY + getNumberSetting().getValue(), getFinishedX() + 5, getFinishedY() + getHeight() / 2 - (Managers.TEXT.getStringHeightI() >> 1), getClickGui().get().enabledTextColor.getValue().getRGB());
        ...
        Render2DUtil.drawBorderedRect(getFinishedX() + 5, getFinishedY() + getHeight() - 2.5f, getFinishedX() + 5 + length, getFinishedY() + getHeight() - 0.5f, 0.5f, hovered ? getClickGui().get().color.getValue().brighter().getRGB() : getClickGui().get().color.getValue().getRGB(), getClickGui().get().outlineColor.getValue().getRGB());
        if (sliding) {
