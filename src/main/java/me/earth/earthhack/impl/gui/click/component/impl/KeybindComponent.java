...
public class KeybindComponent extends SettingComponent<Bind, BindSetting> {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Render2DUtil.drawBorderedRect(getFinishedX() + 4.5f, getFinishedY() + 1.0f, getFinishedX() + getWidth() - 4.5f, getFinishedY() + getHeight() - 0.5f, 0.5f, hovered ? getClickGui().get().hoverColor.getValue().getRGB() : 0, getClickGui().get().outlineColor.getValue().getRGB());
        Managers.TEXT.drawStringWithShadow(isBinding() ? "Press a key..." : getBindSetting().getName() + ": " + ChatFormatting.GRAY+ getBindSetting().getValue(), getFinishedX() + 6.5f, getFinishedY() + getHeight() - Managers.TEXT.getStringHeightI() - 1f, getClickGui().get().enabledTextColor.getValue().getRGB());
    }
