...
public class ModulesFrame extends Frame {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Render2DUtil.drawRect(getPosX(), getPosY(), getPosX() + getWidth(), getPosY() + getHeight(), CLICK_GUI.get().headerColor.getValue().getRGB());
        Render2DUtil.drawBorderedRect(getPosX(), getPosY(), getPosX() + getWidth(), getPosY() + getHeight(), 0.5f, 0, CLICK_GUI.get().outlineColor.getValue().getRGB());
        Managers.TEXT.drawStringWithShadow(getLabel(), getPosX() + 3, getPosY() + getHeight() / 2 - (Managers.TEXT.getStringHeightI() >> 1), CLICK_GUI.get().enabledTextColor.getValue().getRGB());
        if (CLICK_GUI.get().size.getValue()) {
            String disString = "[" + getComponents().size() + "]";
            Managers.TEXT.drawStringWithShadow(disString, (getPosX() + getWidth() - 3 - Managers.TEXT.getStringWidth(disString)), (getPosY() + getHeight() / 2 - (Managers.TEXT.getStringHeightI() >> 1)), CLICK_GUI.get().enabledTextColor.getValue().getRGB());
        }
        if (isExtended()) {
            ...
            Render2DUtil.drawRect(getPosX(), getPosY() + getHeight(), getPosX() + getWidth(), getPosY() + getHeight() + 1 + (getCurrentHeight()), CLICK_GUI.get().backgroundColor.getValue().getRGB());
            GL11.glPushMatrix();
            GL11.glEnable(GL11.GL_SCISSOR_TEST);
            RenderUtil.scissor(getPosX(), getPosY() + getHeight() + 1, new ScaledResolution(Minecraft.getMinecraft()).getScaledWidth(), getPosY() + getHeight() + scrollMaxHeight + 1);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (isExtended()) {
            boolean inBounds = RenderUtil.mouseWithinBounds(mouseX, mouseY, getPosX(), getPosY() + getHeight(), getWidth(), (Math.min(getScrollCurrentHeight(), scrollMaxHeight)) + 1);
            getComponents().forEach(component -> {
                if (inBounds || (component instanceof ModuleComponent && component.isExtended())) {
